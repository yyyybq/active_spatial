# View Spatial Bench Service
# HTTP/WebSocket service wrapper for the View Spatial environment.

from typing import Dict, List, Tuple, Optional, Any
from concurrent.futures import ThreadPoolExecutor, as_completed

from vagen.env.base.base_service import BaseService
from vagen.env.view_spatial.env import ViewSpatialEnv
from vagen.env.view_spatial.env_config import ViewSpatialEnvConfig
from vagen.server.serial import serialize_observation
from .service_config import ViewSpatialServiceConfig


class ViewSpatialService(BaseService):
    """
    Service class for View Spatial Bench environments.
    Implements batch operations with parallel processing for efficiency.
    """
    
    def __init__(self, config: ViewSpatialServiceConfig):
        """
        Initialize the ViewSpatialService.
        ...
        self.environments: Dict[str, ViewSpatialEnv] = {}
        self.env_configs: Dict[str, ViewSpatialEnvConfig] = {}
        self.config = config
        print(f"[DEBUG] ViewSpatialService initialized with config: {self.config}")
    
    def create_environments_batch(self, ids2configs: Dict[str, Any]) -> None:
        """
        Create multiple View Spatial environments in parallel.
        ...
        def create_single_env(env_id: str, config: Dict[str, Any]) -> Tuple[str, Any, Optional[str]]:
            env_name = config.get('env_name', 'view_spatial')
            if env_name != 'view_spatial':
                return env_id, None, f"Expected environment type 'view_spatial', got '{env_name}'"
            
            try:
                env_config_dict = config['env_config']
                env_config = ViewSpatialEnvConfig(**env_config_dict)
                env = ViewSpatialEnv(env_config)
    ...
    def reset_batch(self, ids2seeds: Dict[str, Any]) -> Dict[str, Tuple[Any, Any]]:
        """
        Reset multiple View Spatial environments in parallel.
    ...
    def step_batch(self, ids2actions: Dict[str, Any]) -> Dict[str, Tuple[Dict, float, bool, Dict]]:
        """
        Take a step in multiple View Spatial environments in parallel.
